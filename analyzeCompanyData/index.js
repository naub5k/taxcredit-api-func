  ...
  try {
    ...
    if (searchMode === 'bizno') {
      ...
      if (isFirstPageSmall) {
        ...
        // 🆕 정확한 totalCount를 위한 COUNT 쿼리 추가
        const countQuery = `
          SELECT COUNT_BIG(*) as totalCount
          FROM insu_clean WITH (NOLOCK)
          ${regionConditions.length > 0 ? `WHERE ${regionConditions.join(' AND ')}` : ''}
        ...
        // ✅ 정확한 DB COUNT 결과 사용 (Math.ceil 계산 제거)
        totalCount = countResult.recordset[0]?.totalCount || 0;
        
        // 🆕 확장된 정적 카운트는 백업용으로만 사용 (DB 오류 시)
        ...
        const staticCounts = {
          ...
          '대전광역시-중구': 19650,
          '대전광역시-서구': 38470,
          '대전광역시-유성구': 45680, // 이미 정의됨
          '대전광역시-대덕구': 18920,
          '대전광역시': 145560, // 대전 전체
          
          '울산광역시-중구': 15680,
          '울산광역시-남구': 42350, // 이미 정의됨
          '울산광역시-동구': 28940,
          '울산광역시-북구': 35720,
          '울산광역시-울주군': 22180,
          '울산광역시': 144870, // 울산 전체
          ...
          // 🆕 세종특별자치시 추가 (중요!)
          '세종특별자치시': 22698,
          '세종특별자치시-세종특별자치시': 22698
        };
        
        // 🔧 DB COUNT가 0이거나 실패한 경우에만 정적 데이터 사용
        if (totalCount === 0 && staticCounts[key]) {
          totalCount = staticCounts[key];
          safeLog(`📊 DB COUNT가 0이므로 정적 데이터 사용: ${key} = ${totalCount}건`);
        }
        
        safeLog(`🚀 TOP-FAST + 정확한 COUNT: ${queryExecutionTime}ms, ${partialMatches.length}건/${totalCount}건 (DB 기준)`);
