...
### 1️⃣ **회사 데이터 조회**
```http
GET http://localhost:7071/api/analyze?bizno=1010818435
```
...
```bash
# 세액공제 분석 확인
curl "http://localhost:7071/api/analyze?bizno=1010818435"

# 결과: 200 OK + 분석 결과 JSON
```
...
```bash
# 시간 측정 포함 요청
time curl "http://localhost:7071/api/analyze?bizno=1010818435"
```
